...
- `BACKEND_ORIGIN` (consigliato) = origine backend senza `/api` finale (es: `https://<backend-domain>`).
- In alternativa: `NEXT_PUBLIC_API_BASE_URL` = origine backend senza `/api` finale.
...
6. Aggiungi `BACKEND_ORIGIN` (o `NEXT_PUBLIC_API_BASE_URL`) in Environment Variables (Production).
7. Deploy.
