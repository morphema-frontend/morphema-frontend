      ...
      <div className="grid gap-4">
        <div className="card">
          ...
          <p className="mt-1 text-sm text-zinc-600">
            Seleziona le skill (puoi averne più di una). Salva per aggiornare il profilo.
          </p>
        </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between">
            <div className="text-sm text-zinc-600">Selected: {selected.length}</div>
          ...
          <div className="mt-4 grid gap-3">
            {byJobType.length === 0 ? <div className="text-sm text-zinc-600">No skills catalog found.</div> : null}
            {byJobType.map(([jobTypeId, skills]) => (
              <div key={jobTypeId} className="rounded-lg border border-zinc-200 p-3">
                <div className="text-sm font-semibold">JobType #{jobTypeId}</div>
                <div className="mt-2 grid gap-2 md:grid-cols-2">
                  {skills.map((s) => (
                    <label key={s.id} className="flex cursor-pointer items-center gap-2 rounded-md border border-zinc-200 bg-zinc-50 p-2">
